...
public class WarningController {
    ...
    public String getWarningList(HttpServletRequest request, HttpServletResponse response) {
        ...
        DataTablePageUtil<Warning> dataTable = new DataTablePageUtil<Warning>(request);
        //还是使用List，方便后期用到
        ...
        if (dataTable.getOrder()[0] != 0) {
            ...
            String orderColumn = DatabaseUtil.getColumnName("warning", dataTable.getOrder()[0]);
        ...
        if (dataTable.getSearch() != null && !dataTable.getSearch().equals("")) {
            ...
            warnings = warningService.getWarningListBySearch("%" + dataTable.getSearch() + "%", orderBy);
        }
        //否
        if (!isSearch) {
        ...
        //用PageInfo对结果进行包装
        PageInfo<Warning> pageInfo = new PageInfo<Warning>(warnings);

        //封装数据给DataTables
        dataTable.setDraw(dataTable.getDraw());
//        System.out.println("获得的data:" + dataTable.getOrder_dir());
//        for (Warning t : pageInfo.getList()) {
//            System.out.println("name===" + t.getWarningEvent());
//        }
